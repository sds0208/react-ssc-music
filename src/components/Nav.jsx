  ...
  return (
    <>
      <nav>
        ...
        <div className={`${mobileMenuOpen ? 'hamburger-nav open' : 'hamburger-nav'}`} onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
          <div className="hamburger-line line-1"></div>
          <div className="hamburger-line line-2"></div>
          <div className="hamburger-line line-3"></div>
        </div>
        <div className={`${mobileMenuOpen ? 'links-container open' : 'links-container'}`}>
          <NavLink to="/" className={linkClass}>Home</NavLink>
          <NavLink to="/samples" className={linkClass}>Music</NavLink>
          <NavLink to="/about" className={linkClass}>About</NavLink>
          <NavLink to="/contact" className={({ isActive }) => isActive ? 'contact link active-link' : 'contact link'}>Contact</NavLink>
        </div>
      </nav>
    </>
  )
